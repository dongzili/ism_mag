...
def check_lr(pdf, spec_type = 'dy',namel =  'gb057_1.input_baseline257_freq_00_pol_LL.rebint', namer =  'gb057_1.input_baseline257_freq_00_pol_RR.rebint',log = 0,cmap='Dark2'):
	...
	if spec_type == 'dy':
		...
		if log != 0:
			...
			diff = np.absolute(datal)**2. - np.absolute(datar)**2.
			diff = np.sign(diff)*(np.absolute(diff))**0.5

		del datal,datar
		plt.figure(1)
		plt.imshow(diff, origin='lower',interpolation='none',aspect='auto',cmap=cmap)
		plt.xlim(300,1000)
		plt.ylim(5000,12000)
		plt.title(namel.replace('.rebint','.sqrt'))
